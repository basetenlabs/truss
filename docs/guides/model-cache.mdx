...
- `repo_id` (required): The endpoint for your cloud bucket. Currently, we support Hugging Face, Google Cloud Storage, and Amazon Web Services S3.
...
<Steps>
  ...
  <Step title="Add it to Baseten Secrets Manager">
    Paste your API key in your [secrets manager in Baseten](https://app.baseten.co/settings/secrets) under the key `hf_access_token`. You can read more about secrets [here](/guides/secrets).
  </Step>
...
If you are accessing a public S3 bucket, you can ignore the subsequent steps, but make sure you set an appropriate appropriate policy on your bucket. Users should be able to list and view all files. Otherwise, the model build will fail.
