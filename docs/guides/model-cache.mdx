...
<Steps>
  ...
  <Step title="Add it to Baseten Secrets Manager">
    Paste your API key in your [secrets manager in Baseten](https://app.baseten.co/settings/secrets) under the key `hf_access_token`.
  </Step>
...
Make sure that the key `secrets` only shows up once in your `config.yml`. If you are testing Truss builds locally, you can paste your actual `hf_access_token` here. 
  </Step>
</Steps>
...
Similar to above, you do not need to do anything else for public GCS buckets.
