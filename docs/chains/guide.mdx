...
Truss Chains additionally has a [local debugging mode](chains/guide#local-debugging) which allows you to iterate
more quickly.
...
if __name__ == "__main__":
    with chains.run_local():
        ...
        print(hello_world_chain.run_remote(max_value=5))
...
# Chain with Chainlets using different Resources
...
class PoemGenerator(chains.ChainletBase):
    ...
    def __init__(
        self,
        context: chains.DeploymentContext = chains.depends_context(),
        mistral_llm: MistralLLM = chains.depends(MistralLLM),
    ) -> None:
        super().__init__(context)
...
Our Chain includes the `MistralLLM` Chainlet which requires heavy hardware and python dependencies. These
 ...
 `PoemGenerator`, contains simpler "coordination" logic and has less demanding requirements.
