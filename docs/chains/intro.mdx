...
To ensure that both of these modes, **you must reference other chainlets in a very particular way, using the
Truss Chains Depedency Injection framework**. We will dig into the exact syntax later in the [Chaining Chainlets](/chains/chaining-chainlets) guide,
but to give a little preview:
...
class OtherChain(chains.ChainletBase):
    ...

class MyChain(chains.ChainletBase):
    def __init__(
        self, 
        context: chains.DeploymentContext = chains.provide_context(),
        # Note that chains.provide(...) syntax is used to reference other chainlets.
        # in this __init__ method.
        other_chain: OtherChain = chains.provide(OtherChain)
    ):
        super().__init__()
        self._other_chain = other_chain

    def run(self, input: str) -> str:
        ...
        other_chain_result = self._other_chain.run(...)
        ...
