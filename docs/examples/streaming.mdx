...
In this example, we will show you how to deploy [Falcon 7B](https://huggingface.co/tiiuae/falcon-7b), an LLM, and stream
the output as it is generated.
...
load and run the model. In this first step, we will generate output normally and return it without streaming
the output.
...
Once we have a model that can produce the LLM outputs using the HuggingFace transformers library,
we can adapt it to support streaming. The key change that needs to happen here is in the `predict`
function.

While in the above example, the `predict` function returns a `Dict` containing the model output,
to stream results, we need to return a Python `Generator` from the `predict` function instead.
This will allow us to return partial results to the user as they are generated.

To produce outputs incrementally for the LLM, we will pass a `TextIteratorStreamer` object to the
`generate` function. This object will return the model output as it is generated. We will then kick
off the generation on a separate thread.

What we return from the `predict` function is a generator that will yield the model output from
the streamer object as it is generated.
