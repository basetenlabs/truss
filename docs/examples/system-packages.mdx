...
[LayoutLM Document QA](https://huggingface.co/impira/layoutlm-document-qa) is a multimodal model that answers questions about provided invoice documents. The model requires a system package `tesseract-ocr`, which we need to include in the model serving environment.
...
LayoutLM Document QA is [a `transformers.pipeline` model](https://huggingface.co/docs/transformers/main_classes/pipelines) which are straightforward to implement in Truss.
