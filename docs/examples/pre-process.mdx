---
title: Taking advantage of the pre/post process methods
description: "Deploy a model that makes use of pre-process"
---

Since many ML models have compute requirements that often involves a GPU,
Truss limits the number of concurrent requests to a model. This ensures that
the GPU does not get overloaded, and that the model can respond to requests.

However, many models, in addition to having a GPU compute component, also have
...
ensure that any IO does not prevent utilization of the GPU on your pod.
