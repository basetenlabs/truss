...
1. Select `vLLM` as a `server`
2. Type `facebook/opt-125M` as a `model`
3. Press the `tab` key when you get to `endpoint`. We're going to use the `Completions` endpoint (more on this below).

The underlying server that we use is OpenAI compatible. If you plan on using the model as a chat model, then select `ChatCompletion`. OPT-125M is not a chat model so we selected `Completion`. Finally, navigate to the directory.
...
You'll notice that there's a `config.yaml`. This is where we'll set the resources and other arguments for the model. Open the file in your favorite editor.
