...
class SpeakerMapBuilder:
    ...
    def correlation(scores1: np.ndarray, scores2: np.ndarray) -> SpeakerMap:
        score_matrix_per_frame = np.stack(  # (local_speakers, num_frames, global_speakers)
            [scores1[:, speaker : speaker + 1] * scores2 for speaker in range(scores1.shape[1])],
            axis=0,
        )
...
class SpeakerMap:
    def __init__(self, mapping_matrix: np.ndarray, objective: MappingMatrixObjective):
        ...
        self.mapped_source_speakers = self.objective.mapped_indices(self.mapping_matrix, axis=1)
        self.mapped_target_speakers = self.objective.mapped_indices(self.mapping_matrix, axis=0)
        self._opt_assignments: Optional[List[int]] = None

    @property
    def _raw_optimal_assignments(self) -> List[int]:
        if self._opt_assignments is None:
            self._opt_assignments = self.objective.optimal_assignments(self.mapping_matrix)
    ...
    def unmap_source_speaker(self, src_speaker: int):
        new_cost_matrix = np.copy(self.mapping_matrix)
        new_cost_matrix[src_speaker] = self.objective.invalid_tensor(shape=self.num_target_speakers)
        return SpeakerMap(new_cost_matrix, self.objective)
