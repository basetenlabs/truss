...
class DockerURLs:
    def __init__(self, base_url):
        ...
        self.websockets_url = f"{base_url}/v1/websocket"
...
class TrussHandle:
    ...
    def docker_run(
        ...
        if not detach and not local_port:
            raise ValueError(
                "If the container is not detached, a local port must be provided."
            )
    ...
    try:
        ...
        if wait_for_server_ready:
            if model_server_stop_retry_override is not None:
                _wait_for_model_server(
                    urls.predict_url, stop=model_server_stop_retry_override
                )
            else:
                _wait_for_model_server(urls.predict_url)
