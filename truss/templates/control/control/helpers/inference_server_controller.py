...
class InferenceServerController:
    ...
    def apply_patch(self, patch_request):
        with self._lock:
            ...
            # When hot_reload is set on all patch bodies, skip process
            # stop/start — files are written in place and the caller signals
            # the running inference server to pick up the changes.
            hot_reload = all(getattr(p.body, "hot_reload", False) for p in patches)
