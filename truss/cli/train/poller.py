...
class TrainingPollerMixin:
    ...
    def _maybe_update_poll_stop_time(self) -> None:
        if (
            self._current_status not in JOB_RUNNING_STATES
    ...
    def after_polling(self) -> None:
        if self._current_status in STATES_WITH_ERROR_MESSAGES and self._error_message:
            self.console.print(self._error_message, style="red")
        ...
        elif self._current_status == "TRAINING_JOB_DEPLOY_FAILED":
    ...
    def _update_from_current_status(self) -> None:
        ...
        self._current_status = current_job["training_job"]["current_status"]
        self._error_message = current_job["training_job"].get("error_message")
