...
def _ensure_loop_checkpoint_details(
    ...
    Each entry in ``loops_checkpoint_ids`` is a Loops-checkpoint PK.
    The server resolves it to its run + checkpoint_name on deploy and
    reads the actual weight format off the row — the CLI doesn't need
    to know it.
    """
