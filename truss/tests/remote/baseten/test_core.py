...
class TestOidcFeatureFlagValidation:
    """Tests for OIDC feature flag validation during truss push."""

    def test_config_uses_oidc_auth_docker_aws_oidc(self):
        """Test detection of AWS OIDC in docker_auth."""
        config = TrussConfig(
        ...
        assert _config_uses_oidc_auth(config) is True

    def test_config_uses_oidc_auth_docker_gcp_oidc(self):
        """Test detection of GCP OIDC in docker_auth."""
        config = TrussConfig(
            base_image=BaseImage(
                image="us-west2-docker.pkg.dev/my-project/my-repo/my-image:latest",
                docker_auth=DockerAuthSettings(
    ...
    def test_config_uses_oidc_auth_weights_aws_oidc(self):
        """Test detection of AWS OIDC in weights configuration."""
        config = TrussConfig(
            weights=Weights(
                [
                    WeightsSource(
    ...
    def test_config_uses_oidc_auth_weights_gcp_oidc(self):
        """Test detection of GCP OIDC in weights configuration."""
        config = TrussConfig(
            weights=Weights(
                [
                    WeightsSource(
    ...
    def test_config_uses_oidc_auth_no_oidc(self):
        """Test that non-OIDC configs return False."""
        config = TrussConfig()
        assert _config_uses_oidc_auth(config) is False

    def test_config_uses_oidc_auth_aws_iam(self):
        """Test that AWS_IAM (non-OIDC) returns False."""
        config = TrussConfig(
            base_image=BaseImage(
                image="123456789.dkr.ecr.us-west-2.amazonaws.com/my-image:latest",
                docker_auth=DockerAuthSettings(
    ...
    def test_validate_oidc_feature_flag_enabled(self):
        """Test that validation passes when OIDC flag is enabled."""
        api = MagicMock()
        api.get_organization_feature_flags.return_value = [OIDC_FEATURE_FLAG]
        ...
        # Should not raise
        validate_oidc_feature_flag(api, config)
        api.get_organization_feature_flags.assert_called_once()

    def test_validate_oidc_feature_flag_not_enabled(self):
        """Test that validation fails when OIDC is used but flag is not enabled."""
        api = MagicMock()
        api.get_organization_feature_flags.return_value = []
        ...
        assert "OIDC authentication" in str(exc_info.value)
        assert "not enabled for your organization" in str(exc_info.value)

    def test_validate_oidc_feature_flag_no_oidc_used(self):
        """Test that validation is skipped when OIDC is not used."""
        api = MagicMock()
        config = TrussConfig()

        # Should not raise and should not call the API
        validate_oidc_feature_flag(api, config)
        api.get_organization_feature_flags.assert_not_called()

    def test_validate_oidc_feature_flag_weights_not_enabled(self):
        """Test that validation fails for weights OIDC when flag is not enabled."""
        api = MagicMock()
        api.get_organization_feature_flags.return_value = ["OTHER_FLAG"]
