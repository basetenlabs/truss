...
def test_deactivate_loops_run_falls_back_to_deployment_on_404(baseten_api):
    # Older backends lack the run-scoped route: a 404 makes the client resolve
    # the run to its deployment and deactivate that instead.
    not_found = requests.exceptions.HTTPError(response=mock.Mock(status_code=404))
    mock_rest_client = mock.Mock()
    # First POST (run endpoint) 404s; second POST (deployment deactivate) succeeds.
    mock_rest_client.post.side_effect = [not_found, None]
    mock_rest_client.get.return_value = {
        "run": {"id": "run-1", "deployment_id": "deploy-9"}
    }
    baseten_api._rest_api_client = mock_rest_client
    ...
    assert (
        mock_rest_client.post.call_args_list[0][0][0]
        == "v1/loops/runs/run-1/deactivate"
    )
    assert mock_rest_client.get.call_args[0][0] == "v1/loops/runs/run-1"
    assert (
        mock_rest_client.post.call_args_list[1][0][0]
        == "v1/loops/deployments/deploy-9/deactivate"
    )


def test_deactivate_loops_run_propagates_non_404_error(baseten_api):
    # A non-404 error is a real failure, not a missing route — don't fall back.
    server_error = requests.exceptions.HTTPError(response=mock.Mock(status_code=500))
    mock_rest_client = mock.Mock()
    mock_rest_client.post.side_effect = server_error
    baseten_api._rest_api_client = mock_rest_client
