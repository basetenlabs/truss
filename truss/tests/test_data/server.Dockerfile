...
RUN env -i PATH="$PATH" HOME="$HOME" UV_HTTP_TIMEOUT=${UV_HTTP_TIMEOUT:-300} uv pip install --system --break-system-packages --index-strategy unsafe-best-match --python /usr/local/bin/python3 -r base_server_requirements.txt --no-cache-dir
COPY --chown=root:root ./constraints.txt constraints.txt
COPY --chown=root:root ./requirements.txt requirements.txt
RUN env -i PATH="$PATH" HOME="$HOME" UV_HTTP_TIMEOUT=${UV_HTTP_TIMEOUT:-300} uv pip install --system --break-system-packages --index-strategy unsafe-best-match --python /usr/local/bin/python3 -r requirements.txt -c constraints.txt --no-cache-dir
WORKDIR $APP_HOME
