...
@pytest.mark.anyio
async def test_hot_reload(truss_container_fs, helpers, connected_request):
...
class Model:
    ...
    def predict(self, request):
        return {"version": "v1", "state": self.state, "init_counter": self.init_counter}
"""
    updated_model = """
class Model:
    def predict(self, request):
        return {"version": "v2", "state": self.state, "init_counter": self.init_counter}
"""
    with (
        _clear_model_load_modules(),
        helpers.file_content(model_file, original_model),
        helpers.sys_path(app_path),
    ):
        model_wrapper_module = importlib.import_module("model_wrapper")
        ModelWrapper = getattr(model_wrapper_module, "ModelWrapper")

        config = yaml.safe_load((app_path / "config.yaml").read_text())
        os.chdir(app_path)
        wrapper = ModelWrapper(config, sdk_trace.NoOpTracer())
        wrapper.load()

        # Verify original behavior
        result = await wrapper.predict({}, connected_request)
        assert result["version"] == "v1"
        assert result["state"] == "loaded"
        assert result["init_counter"] == 1

        # Write new code to disk and hot-reload
        model_file.write_text(updated_model)
        wrapper.hot_reload()

        # Verify new code is active but state is preserved
        # (init_counter and state survive because __init__/load are not re-run)
        result = await wrapper.predict({}, connected_request)
...
@pytest.mark.anyio
async def test_hot_reload_syntax_error(truss_container_fs, helpers):
    app_path = truss_container_fs / "app"
    model_file = app_path / "model" / "model.py"
...
"""
    with (
        ...
        # Write broken code and attempt hot-reload
        model_file.write_text("def broken(:\n")
        with pytest.raises(SyntaxError):
            wrapper.hot_reload()

        # Model should still work with old code's class
        # (the __class__ swap never happened)
        assert wrapper._maybe_model is not None
