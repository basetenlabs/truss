...
from truss.cli.train.core import view_cache_summary
...
def test_view_cache_summary_success(capsys):
    ...
    # Test the function
    view_cache_summary(mock_remote, "proj123")
    ...
    assert "100.00 MB" in captured.out
    assert "1024 B" in captured.out
    assert "Total files: 2" in captured.out
    assert "Total size: 100.00 MB" in captured.out
