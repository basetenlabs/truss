...
class InternalWebhook(chains.ChainletBase):
    ...
    async def run_remote(self, result: data_types.TranscriptionExternal) -> None:
        row_data = {
            ...
            async for attempt in retrying:
                with attempt:
                    if attempt.retry_state.attempt_number > 1:
                        logging.warning(
                            f"Retrying `{self._transcribe}`: {attempt.retry_state}."
                        )
...
class EnqueueBatch(chains.ChainletBase):
    ...
    def __init__(
        ...
        internal_webhook: InternalWebhook = chains.depends(InternalWebhook),
