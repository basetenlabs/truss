...
def _push_service_docker(
    ...
    th.add_secret(
        public_types._CHAIN_API_KEY_SECRET_NAME, options.baseten_chain_api_key
    )
...
def _create_chains_secret_if_missing(remote_provider: b10_remote.BasetenRemote) -> None:
    ...
    if public_types._CHAIN_API_KEY_SECRET_NAME not in secret_names:
        logging.info(
            "It seems you are using chains for the first time, since there "
            f"is no `{public_types._CHAIN_API_KEY_SECRET_NAME}` secret on baseten. "
        ...
        workspace_api_key = remote_provider.api.create_api_key(
            ...
            name=public_types._CHAIN_API_KEY_NAME,
        )["api_key"]

        remote_provider.api.upsert_secret(
            public_types._CHAIN_API_KEY_SECRET_NAME, workspace_api_key
        )
