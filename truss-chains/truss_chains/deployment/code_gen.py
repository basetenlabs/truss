...
def _gen_truss_config(
    ...
    if public_types._CHAIN_API_KEY_SECRET_NAME not in config.secrets:
        config.secrets[public_types._CHAIN_API_KEY_SECRET_NAME] = (
            public_types.SECRET_DUMMY
        )
    else:
        logging.info(
            f"Chains automatically add {public_types._CHAIN_API_KEY_SECRET_NAME} "
