...
class TestPublicApiDisableTrussDownload:
    ...
    def mock_service(self):
        ...
        class MockBasetenChainService(BasetenChainService):
            def __init__(self):
                ...
                super().__init__(
                    name="test_chain", chain_deployment_handle=Mock(), remote=Mock()
                )
    ...
    def test_push_with_disable_truss_download_true(
        self, mock_push, mock_entrypoint, mock_service
    ):
    ...
    @patch("truss_chains.public_api.deployment_client.push")
    def test_push_with_disable_truss_download_false(
        self, mock_push, mock_entrypoint, mock_service
    ):
    ...
    @patch("truss_chains.public_api.deployment_client.push")
    def test_push_without_disable_truss_download_defaults_to_false(
        self, mock_push, mock_entrypoint, mock_service
    ):
        ...
        result = public_api.push(entrypoint=mock_entrypoint, chain_name="test_chain")
    ...
    @patch("truss_chains.public_api.deployment_client.push")
    def test_push_with_all_parameters_including_disable_truss_download(
        self, mock_push, mock_entrypoint, mock_service
    ):
    ...
    def test_push_options_baseten_create_called_with_correct_parameters(
        self, mock_entrypoint, mock_service
    ):
        ...
        with patch("truss_chains.public_api.deployment_client.push") as mock_push:
            with patch(
                "truss_chains.public_api.private_types.PushOptionsBaseten.create"
            ) as mock_create:
                mock_push.return_value = mock_service
