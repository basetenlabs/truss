...
class Model:
    ...
    def predict(self, request: Dict) -> Dict[str, List]:
        return {
            "predictions": [
                self._predict_single(instance) for instance in request["inputs"]
            ]
        }
    ...
    def _predict_single(self, instance: Dict) -> str:
        ...
        content_image = Image.open(urllib.request.urlopen(content_image_url)).convert(
            "RGB"
        )
        style_image = Image.open(urllib.request.urlopen(style_image_url)).convert("RGB")

        content_image = (
            np.asarray(content_image).astype(np.float32)[np.newaxis, ...] / 255.0
        )
        style_image = (
            np.asarray(style_image).astype(np.float32)[np.newaxis, ...] / 255.0
        )

        content_image = tf.image.resize(
            content_image, IMG_SIZE_CONSTRAINT, preserve_aspect_ratio=True
        )
