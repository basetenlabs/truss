...
def extract_invoice_data(document):
    ...
    for entity in document.entities:
        bounding_boxes = get_line_bbox_for_segment(
            ...
            entity.text_anchor.text_segments[-1].end_index,
            document_lines,
        )
        extract = {
            "field": entity.type_,
        ...
        if (
            extract["field_confidence"] > CONFIDENCE_THRESHOLD
            and extract["field_value_confidence"] > CONFIDENCE_THRESHOLD
        ):
    ...
    x_min = min(
        [
            ...
            for vertex in min(
                matching_lines,
                key=lambda x: [
                    vertex.x for vertex in x.layout.bounding_poly.normalized_vertices
                ],
    ...
    y_min = min(
        [
            ...
            for vertex in min(
                matching_lines,
                key=lambda x: [
                    vertex.y for vertex in x.layout.bounding_poly.normalized_vertices
                ],
            ).layout.bounding_poly.normalized_vertices
        ]
    )
