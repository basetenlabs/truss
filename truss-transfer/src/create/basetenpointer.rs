    ...
    async fn test_create_basetenpointer_hf_julien_dummy() {
        ...
        match result {
            Ok(manifest_json) => {
                ...
                let manifest: Vec<Value> = serde_json::from_str(&manifest_json)
                    .expect("Failed to parse manifest JSON");
                
                let pretty_json = serde_json::to_string_pretty(&manifest)
                ...
                let required_fields = [
                    "resolution", "uid", "file_name", "hashtype", "hash", "size", "runtime_secret_name"
                ];
                ...
                for pointer in &manifest {
                    ...
                    for field in &required_fields {
                        assert!(
                            pointer.get(field).is_some(),
                            "Missing field: {}", field
                        );
                    }

                    // Check resolution fields
                    let resolution = pointer.get("resolution")
                    ...
                    let resolution_type = resolution.get("resolution_type")
                        .and_then(|v| v.as_str())
                    ...
                    let runtime_secret = pointer.get("runtime_secret_name")
                        .and_then(|v| v.as_str())
                        .expect("Runtime secret name should be a string");
                    assert_eq!(runtime_secret, "hf_access_token_2");

                    // Verify the UID format
                    let uid = pointer.get("uid")
                        .and_then(|v| v.as_str())
                        .expect("UID should be a string");
                    assert!(uid.starts_with("julien-c/dummy-unknown:"), "UID should start with repo:revision");

                    // Verify file name starts with the expected path
                    let file_name = pointer.get("file_name")
                        .and_then(|v| v.as_str())
                        .expect("File name should be a string");
                    assert!(file_name.starts_with("/app/model_cache/julien_dummy/"), 
                           "File name should start with correct path: {}", file_name);
                }

                println!("✓ BasetenPointer structure validation passed");
                println!("✓ Found {} file(s) in the HuggingFace repository", manifest.len());

                // Verify we got some specific expected files
                let file_names: Vec<String> = manifest.iter()
                    .filter_map(|p| p.get("file_name")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string()))
                    .collect();
                ...
                for expected_file in &expected_files {
                    ...
                    assert!(found, "Expected file {} not found in manifest", expected_file);
                }
            }
            Err(e) => {
                ...
                if e.to_string().contains("Failed to create HuggingFace API client") 
